...
public abstract class DexCount {
	...
	DexCount(OutputStyle outputStyle) {
		...
		packageCount = this.outputStyle == OutputStyle.FLAT ? new TreeMap<String, IntHolder>() : null;
		sb = new StringBuilder();
	}

	public abstract void generate(DexData dexData, boolean includeClasses, String packageFilter, int maxDepth,
			Filter filter);
	...
	static class Node {
		...
		void output(String indent) {
			...
			for (String name : children.navigableKeySet()) {
				...
				sb.append(indent + name + ": " + child.count+"\n");
				child.output(indent);
			}
		}
	}
